...
const SPACE = " ";
const NEW_LINE = "\r\n";
const CUILLOTINE = NEW_LINE + NEW_LINE;
const START_LINE_NAMES = ["method", "url", "version"];


//헤더가 어디까지인지 인덱스 찾을 함수
const getHeaderEndIndex = (request) => request.indexOf(CUILLOTINE);
...
const getRequest = (buffer) => {
  ...
  const [headerString, ...bodyString] = buffer.toString().split(CUILLOTINE)
  const body = bodyString.join(CUILLOTINE); //문자열로 변환
