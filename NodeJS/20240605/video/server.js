...
// path : 파일의 경로를 편하게 사용하기 위해서

// path  : 파일의 경로를 지정할때 사용을 한다.
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === "/video") {
    ...
    if (headerRange) {
      ...
      // 비디오의 청크 크기를 정합시다.
      const chunksize = 3 * 1024; // 3KB  3말고 64로 수정하면 빠르게 됨
      ...
      // 응답 헤더 내용 작성
      // 206 성공인데 리소스가 일부분 제공한다. 일부분 전달했고 성공이야
      ...
      // 바이트 범위 지원하고 있어
      // Content-Length : 이 응답에서 전달하는 데이터의 길이 지금은 청크단위로 우리가 데이터를 제곧하고 있다.
      // Content-Type : 제공하는 콘텐츠의 타입
      // 응답 내용으로 추가
      // 스트림 쓰는거 
    ...
    // 저는요 스트리밍 하고싶지 않고 파일을 그냥 받고 싶습니다.
  } else {
    res.writeHead(404);
    res.end()
  }
